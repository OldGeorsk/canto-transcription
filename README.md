...
| `--model` | Whisper 模型名称 | `large-v3` | `--model large-v3` |
| `--language` | 语言代码 | `yue`（粤语） | `--language zh` |
| `--device` | 计算设备 | `cuda`（GPU） | `--device cpu` |
| `--batch-size` | 批处理大小 | 16 | `--batch-size 8`（显存不足时调小） |
| `--chunk-size` | 音频分段长度（秒） | 30 | `--chunk-size 20` |
| `--compute-type` | 计算精度 | `float16` | `--compute-type int8` |
...
本项目早期曾内置实验性的 Qwen3-ASR 后端作为 WhisperX 的替代引擎。**目前已停用**：不再随仓库提供其依赖文件（`requirements-qwen.txt`）、配置文件（`transcribe_config_qwen.json`）与模型下载。

- 当前正式支持、推荐的 ASR 引擎仅有 **WhisperX**（默认）。
- `transcribe.py` 源码中仍保留 `--backend qwen` 的代码路径（休眠状态），但**未提供运行所需的依赖与配置，无法直接使用**。
- 若未来确有需要，可从仓库 git 历史中恢复上述文件，或参考 `transcribe.py` 中 `_transcribe_qwen()` 的实现自行配置。
...
| 配置文件 | 写入命令 |
|----------|----------|
| `transcribe_config_whisperx.json` | `python transcribe.py --save-config` |
